...
export default function AdminPage() {
      ...
      <main className="min-h-screen flex items-center justify-center px-4">
        <section className="w-full max-w-sm rounded-xl border border-[var(--color-ink-700)] bg-[rgba(0,0,0,0.35)] p-5">
          ...
          <form onSubmit={onLogin} className="flex flex-col gap-3">
            ...
            <p className="text-[var(--color-ink-500)] text-[10px] text-center mt-1">
  ...
  return (
    <div className="min-h-screen bg-[#07070E] px-4 py-6 max-w-2xl mx-auto">
      ...
      {/* Tabs */}
      <div className="grid grid-cols-2 gap-2 mb-6 bg-[#0F0F1A] rounded-xl p-1">
        {(['ventas', 'setup'] as DashTab[]).map(t => (
          <button
            key={t}
            onClick={() => setTab(t)}
          ...
          >
            {t === 'ventas' ? '🚀 Ventas' : '⚙️ Setup cliente'}
          </button>
        ))}
      </div>
